...
def view_to_region_scaled(context, x, y):
    scale = get_dpi_factor(context)
    region = context.region
    return region.view2d.view_to_region(x * scale, y * scale, clip=False)
...
def draw_circle_batch(x, y, radius, color):
    shader = get_shader('UNIFORM_COLOR')
    if not shader: return
    vertices = [(x, y)]
    indices = []
...
def draw_lines_batch(points, color, thickness=2.0):
...
def draw_texture_batch(texture, x, y, width, height):
...
def draw_arrow_head(start_point, end_point, color, size=10.0, retreat=8.0):
    shader = get_shader('UNIFORM_COLOR')
    if not shader: return
    x1, y1 = start_point
    x2, y2 = end_point
