...
def paste_image_from_clipboard():
    try:
        ...
        if sys.platform == "win32":
            cmd = ["powershell", "-WindowStyle", "Hidden", "-command", f"Add-Type -AssemblyName System.Windows.Forms;$i=[System.Windows.Forms.Clipboard]::GetImage();if($i){{$i.Save('{path}',[System.Drawing.Imaging.ImageFormat]::Png);exit 0}}exit 1"]
            subprocess.run(cmd)
        elif sys.platform == "darwin": subprocess.run(["pngpaste", path])
        elif sys.platform.startswith("linux"): 
            with open(path, "wb") as f: subprocess.run(["xclip", "-selection", "clipboard", "-t", "image/png", "-o"], stdout=f)
...
class NODE_OT_paste_image(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if path:
            ...
            return {'FINISHED'}
        self.report({'WARNING'}, "无图片"); return {'CANCELLED'}
...
def draw_ui_layout(layout, context, draw_footer=True, draw_sequence=True):
    ...
    row.scale_y = 0.8
    row.prop(context.scene, "na_tag_mode_prepend", text="", icon='ALIGN_LEFT' if context.scene.na_tag_mode_prepend else 'ALIGN_RIGHT', toggle=True)
    for t in ["★", "⚠", "?", "!"]: 
        op = row.operator("node.na_add_quick_tag", text=t); op.tag_text = t
    ...
    def_colors = [
        (0.6, 0.1, 0.1, 0.9), (0.2, 0.5, 0.2, 0.9), (0.2, 0.3, 0.5, 0.9),
        (0.8, 0.35, 0.05, 0.9), (0.4, 0.1, 0.5, 0.9), (0.0, 0.0, 0.0, 0.0)
    ]
    ...
    row = box.row(align=True)
    row.prop(node, "na_text_color", text=""); row.prop(node, "na_bg_color", text="")
...
class NODE_OT_na_quick_edit(bpy.types.Operator):
    ...
    bl_options = {'REGISTER', 'UNDO'}
    @classmethod
    def poll(cls, context): return context.active_node is not None
    ...
    def draw(self, context): draw_ui_layout(self.layout, context, draw_sequence=False)
...
def register():
    for c in [NODE_OT_reset_img_width, NODE_OT_toggle_text_width_link, NODE_OT_reset_offset, NODE_OT_reset_img_offset, NODE_OT_open_image, NODE_OT_paste_image, NODE_OT_apply_preset, NODE_OT_copy_active_to_selected, NODE_OT_add_quick_tag, NODE_OT_copy_node_label, NODE_OT_na_quick_edit]:
...
def unregister():
    ...
    for c in [NODE_OT_na_quick_edit, NODE_OT_copy_node_label, NODE_OT_add_quick_tag, NODE_OT_copy_active_to_selected, NODE_OT_apply_preset, NODE_OT_paste_image, NODE_OT_open_image, NODE_OT_reset_img_offset, NODE_OT_reset_offset, NODE_OT_toggle_text_width_link, NODE_OT_reset_img_width]:
