...
class NODE_OT_reset_prefs(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.target_section == 'SEQ':
            ...
            for p in ["pref_text_default_size", "pref_text_default_color", "pref_bg_default_color", "pref_text_default_fit", "pref_text_default_align"]:
                prefs.property_unset(p)
            for i in range(1, 7): 
...
class NODE_AddonPreferences(bpy.types.AddonPreferences):
    ...
    pref_seq_bg_color: bpy.props.FloatVectorProperty(name="圆背景色", subtype='COLOR', size=4, default=(0.8, 0.1, 0.1, 1.0), min=0, max=1, update=tag_redraw)
    pref_seq_font_size: bpy.props.IntProperty(name="数字字号", default=8, min=4, max=100, update=tag_redraw)
    pref_seq_font_color: bpy.props.FloatVectorProperty(name="数字颜色", subtype='COLOR', size=4, default=(1.0, 1.0, 1.0, 1.0), min=0, max=1, update=tag_redraw)
    ...
    pref_text_default_align: bpy.props.EnumProperty(
        name="默认对齐", items=[('TOP', "顶部", ""), ('BOTTOM', "底部", ""), ('LEFT', "左侧", ""), ('RIGHT', "右侧", "")], default='TOP'
    )
    ...
    # 4. 图像设置区
    pref_img_default_align: bpy.props.EnumProperty(
        name="默认对齐", items=[('TOP', "顶部", ""), ('BOTTOM', "底部", ""), ('LEFT', "左侧", ""), ('RIGHT', "右侧", "")], default='TOP'
    )
    pref_img_max_res: bpy.props.EnumProperty(
        name="纹理分辨率限制",
...
def init_props():
    ...
    bpy.types.Node.na_bg_color = bpy.props.FloatVectorProperty(name="背景", subtype='COLOR', size=4, default=(0.2, 0.3, 0.5, 0.9), min=0.0, max=1.0, update=tag_redraw)
    bpy.types.Node.na_text_color = bpy.props.FloatVectorProperty(name="文字", subtype='COLOR', size=4, default=(1.0, 1.0, 1.0, 1.0), min=0.0, max=1.0, update=tag_redraw)
    ...
    bpy.types.Node.na_sequence_color = bpy.props.FloatVectorProperty(name="序号颜色", subtype='COLOR', size=4, default=(0.8, 0.1, 0.1, 1.0), min=0.0, max=1.0, update=tag_redraw)
    ...
    bpy.types.Scene.na_show_global_sequence = bpy.props.BoolProperty(name="显示序号", default=True, update=tag_redraw, description="全局显示或隐藏所有序号")
    ...
    bpy.types.Scene.na_is_interactive_mode = bpy.props.BoolProperty(name="交互模式", default=False, update=update_interactive_mode, description="点击节点即可编号，右键或ESC退出")
    
    bpy.types.Scene.na_sort_by_sequence = bpy.props.BoolProperty(name="按序号排序", default=False, update=tag_redraw, description="勾选后，有序号的节点将优先显示在列表顶部")
...
def clear_props():
    props = ["na_text", "na_font_size", "na_bg_color", "na_text_color", "na_auto_width", "na_bg_width", "na_swap_content_order", "na_image", "na_img_width", "na_show_image", "na_img_width_auto", "na_align_pos", "na_offset", "na_z_order_switch", "na_sequence_index", "na_sequence_color", "na_img_align_pos", "na_img_offset", "na_text_fit_content", "na_show_text", "na_is_initialized"]
    for p in props:
...
class NODE_PT_annotator_gpu_panel(bpy.types.Panel):
    ...
    bl_category = "节点随记"
    @classmethod
    def poll(cls, context): return context.space_data.type == 'NODE_EDITOR'
    def draw(self, context):
        layout = self.layout
        scene = context.scene
        box = layout.box()
...
classes = [NODE_PT_annotator_gpu_panel, NODE_OT_clear_text, NODE_OT_fix_prop, NODE_OT_clear_all_scene_notes, NODE_OT_na_swap_order, NODE_OT_interactive_seq, NODE_OT_clear_global_sequence, NODE_OT_reset_prefs, NODE_AddonPreferences]
